...
const CardTemplate = ({title, image, description, index}) =>{
    ...
    return (
        <div className='border border-neutral_200 rounded-cards h-auto w-full p-0 relative overflow-hidden hover:shadow-[10px_30px_100px_0px_#00000017] hover:bg-white cursor-pointer'>
            ...
            {/* More view */}
            <div onClick={showLess} className='p-6 w-full h-full absolute left-0 bg-green_700 disappear rounded-cards'>
...
export default function Market() {
  return (
    <div className='mt-20 xl:mt-48'>
        ...
        <div className='mt-20 container px-4 lg:px-10 mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5'>
            {marketArr.map((market, index)=>{
                return <CardTemplate index={index} title={market.title} image={market.image} description={market.description}/>
            })}
        </div>
    </div>
  )
